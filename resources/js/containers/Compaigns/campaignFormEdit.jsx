...
import { getEmailTemplateListRequest, getMessageTemplateListRequest, showCampaignRequest } from '../../redux/actions';
const { Option } = Select;
const CompaignForm = ({ onSubmitValues, editCampaignId }) => {
    ...
    const showCampaignReducer = useSelector(state => state.campaignReducer.showCampaign)
    const showCampaignLoading = useSelector(state => state.campaignReducer.loadingShowCampaignButton)
    const loadingCampaignAddButton = useSelector(state => state.campaignReducer.loadingCampaignAddButton)


    useEffect(() => {
        console.log('editCampaignId :>> ', editCampaignId);
        form.resetFields()
        dispatch(showCampaignRequest(editCampaignId))
    ...
    useEffect(() => {
        ...
        console.log('campaignReducer :>> ', showCampaignReducer);
        setRadioSelect(showCampaignReducer && showCampaignReducer.template && showCampaignReducer.template.medium || "Deleted")
        form.setFieldsValue(showCampaignReducer);
    }, [showCampaignReducer])
        ...
        <div>
            <Form onFinish={onFinish} name="compaign" form={form} ref={formRef} >
                ...
                <Form.Item className="mt-2" style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'end', width: '100%' }}>
                    <Button loading={showCampaignLoading || loadingCampaignAddButton} type="primary" htmlType="submit" style={{ display: 'flex', justifyContent: 'end', alignItems: 'end' }}>
                        Update
                    </Button>
                </Form.Item>
            </Form>
